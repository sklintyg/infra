...
public class SjukfallIntygPatientCreatorTest {
    private static final String LOCATION_INTYGSDATA = "classpath:Sjukfall/Patient/intygsdata-patient-creator.csv";
    ...
    @Before
    public void setup() {
        creator = new SjukfallIntygPatientCreator();
    }

    @Test
    public void testCreatingMapWithMaxGlappZeroDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 0, activeDate);
        assertTrue("Expected 4 but was " + map.size(), map.size() == 4);
    }

    @Test
    public void testCreatingMapWithMaxGlappOneDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 1, activeDate);
        assertTrue("Expected 3 but was " + map.size(), map.size() == 3);
    }

    @Test
    public void testCreatingMapWithMaxGlappTwoDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 2, activeDate);
        assertTrue("Expected 3 but was " + map.size(), map.size() == 2);
    }

    @Test
    public void testCreatingMapWithMaxGlappThreeDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 3, activeDate);
        assertTrue("Expected 2 but was " + map.size(), map.size() == 2);
    }

    @Test
    public void testCreatingMapWithMaxGlappNineDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 9, activeDate);
        assertTrue("Expected 2 but was " + map.size(), map.size() == 2);
    }

    @Test
    public void testCreatingMapWithMaxGlappTenDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 10, activeDate);
        assertTrue("Expected 2 but was " + map.size(), map.size() == 1);
    }

    @Test
    public void testCreatingMapWithMaxGlappElevenDays() {
        Map<Integer, List<SjukfallIntyg>> map = creator.createMap(intygDataList, 11, activeDate);
