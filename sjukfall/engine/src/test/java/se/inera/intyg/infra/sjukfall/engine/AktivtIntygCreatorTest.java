...
public class AktivtIntygCreatorTest {

    private static final String LOCATION_INTYGSDATA = "classpath:AktivtIntygCreatorTest/intygsdata-creator.csv";

    private static List<IntygData> intygDataList;

    private AktivtIntygCreator creator;
    ...
    @Before
    public void setup() {
        creator = new AktivtIntygCreator();
    }

    @Test
    public void testCreatingMap() {
        Map<String, List<AktivtIntyg>> map = creator.createMap(intygDataList, activeDate);
    ...
    public void testReducedMap() {
        Map<String, List<AktivtIntyg>> map = creator.createMap(intygDataList, activeDate);
        Map<String, List<AktivtIntyg>> reducedMap = creator.reduceMap(map);
    ...
    public void testSortedMap() {
        Map<String, List<AktivtIntyg>> map = creator.createMap(intygDataList, activeDate);
        Map<String, List<AktivtIntyg>> sortedMap = creator.sortValues(map);

        for (Map.Entry<String, List<AktivtIntyg>> entry : sortedMap.entrySet()) {
            if (entry.getValue().size() > 1) {
                AktivtIntyg[] arr = entry.getValue().toArray(new AktivtIntyg[entry.getValue().size()]);
    ...
    public void testSetActiveCertificate() {
        Map<String, List<AktivtIntyg>> map = creator.createMap(intygDataList, activeDate);
        Map<String, List<AktivtIntyg>> activeMap = creator.setActive(map);
    ...
    public void testFall1() {
        ...
        Map<String, List<AktivtIntyg>> map = creator.create(intygDataList, activeDate);

        List<AktivtIntyg> list = map.get(key);
    ...
    private static void assertStartDate(AktivtIntyg intygsData, String datum) {
        assertTrue(intygsData.getStartDatum().equals(LocalDate.parse(datum)));
    }

    private static void assertEndDate(AktivtIntyg intygsData, String datum) {
