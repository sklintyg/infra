...
public class HsaOrganizationsServiceMockTest {
    ...
    @Test
    public void testPostaddressFormateringForMottagning() throws IOException, HsaServiceCallException {
        when(organizationClient.getUnit(UNIT_HSA_ID, null)).thenReturn(buildGetUnitReponse("EnhetWCTand.xml"));
        Vardenhet vardenhet = service.getVardenhet(UNIT_HSA_ID);
        verify(organizationClient).getUnit(UNIT_HSA_ID, null);
        Assert.assertEquals("Nordic MedTest Bryggaregatan 11", vardenhet.getPostadress());
    }
